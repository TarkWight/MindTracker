...
extension SettingsViewController {
    func presentAvatarPicker() {
        let alert = UIAlertController(
            title: LocalizedKey.avatarPickerTitle,
            message: nil,
        ...
        alert.addAction(
            UIAlertAction(
                title: LocalizedKey.avatarPickerCamera,
                style: .default
            ) { [weak self] _ in
                self?.presentCamera()
            }
        ...
        alert.addAction(
            UIAlertAction(
                title: LocalizedKey.avatarPickerGallery,
                style: .default
            ) { [weak self] _ in
                self?.presentGallery()
            }
        ...
        alert.addAction(
            UIAlertAction(
                title: LocalizedKey.avatarPickerDelete,
                style: .destructive
            ) { [weak self] _ in
        ...
        alert.addAction(
            UIAlertAction(
                title: LocalizedKey.avatarPickerCancel,
                style: .cancel
            )
        )
...
extension SettingsViewController: PHPickerViewControllerDelegate,
    ...
    func imagePickerController(
        ...
        guard
            let image = info[.editedImage] as? UIImage ?? info[.originalImage]
                as? UIImage,
            let data = image.pngData()
        else { return }
