...
final class TagInputView: UIView {
    ...
    private func setup() {
        ...
        iconButton.setImage(
            AppIcons.plus?.withRenderingMode(.alwaysTemplate),
            for: .normal
        )
        ...
        UIView.animate(withDuration: 0.25) {
            if self.isInputMode {
                ...
                let width =
                    ...
                    .size(withAttributes: [.font: font]).width + 32
    ...
    private func updateDismissTapGesture() {
        ...
        if isInputMode {
            let tap = UITapGestureRecognizer(
                target: self,
                action: #selector(handleTapOutside(_:))
            )
...
extension TagInputView: UITextFieldDelegate {
    ...
    func textFieldDidChangeSelection(_ textField: UITextField) {
        ...
        let width =
            (text as NSString).size(withAttributes: [.font: font]).width + 32
