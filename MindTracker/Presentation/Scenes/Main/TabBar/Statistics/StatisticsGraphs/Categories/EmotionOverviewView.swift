...
final class EmotionOverviewView: UIView {
    ...
    func configure(with data: [EmotionCategory: Int]) {
        ...
        let rawNormalized: [(UIColor, Double)] = filteredData
            .map { ($0.key.color, Double($0.value) / Double(total) * 100) }

        let rounded = rawNormalized.map { ($0.0, Int($0.1)) }
        let sum = rounded.reduce(0) { $0 + $1.1 }

        let remainders = rawNormalized
            .enumerated()
            .map { (index, pair) in
                (index, pair.1 - Double(Int(pair.1)))
            }
            .sorted { $0.1 > $1.1 }

        var final: [(UIColor, Int)] = rounded
    ...
    private func drawCircle(in context: CGContext, circle: Circle) {
        guard let gradient = CGGradient(
            ...
            colors: gradientColors(from: circle.color) as CFArray,
            locations: [0, 1]
        ) else {
            return
        }
    ...
    private func calculateCircles(with data: [(UIColor, Int)]) -> [Circle] {
        ...
        for (index, (color, percentage)) in sortedData.enumerated() {
            ...
            let circle = Circle(center: center, radius: radius, color: color, percentage: percentage)
    ...
    private func centerTemplate(for count: Int) -> [CGPoint] {
        let templates: [[CGPoint]] = [
            ...
            [CGPoint(x: 0.35, y: 0.35), CGPoint(x: 0.65, y: 0.65), CGPoint(x: 0.65, y: 0.35)],
            [CGPoint(x: 0.35, y: 0.35), CGPoint(x: 0.65, y: 0.65), CGPoint(x: 0.65, y: 0.35), CGPoint(x: 0.35, y: 0.65)]
        ]
        return templates[min(count, templates.count - 1)]
    }

    private func findValidCenter(using context: CirclePlacementContext) -> CGPoint {
        ...
        for _ in 0..<context.maxAttempts {
            ...
            let candidate = CGPoint(
                x: clamp(context.templatePoint.x * width + offsetX, min: context.radius, max: width - context.radius),
                y: clamp(context.templatePoint.y * height + offsetY, min: context.radius, max: height - context.radius)
            )

            if isCenterValid(candidate, radius: context.radius, existing: context.existingCircles, multiplier: context.minDistanceMultiplier) {
                return candidate
            }
        }

        return CGPoint(
            x: clamp(context.templatePoint.x * width, min: context.radius, max: width - context.radius),
            y: clamp(context.templatePoint.y * height, min: context.radius, max: height - context.radius)
        )
    }

    private func isCenterValid(_ center: CGPoint, radius: CGFloat, existing: [Circle], multiplier: CGFloat) -> Bool {
        for other in existing {
            let dist = hypot(other.center.x - center.x, other.center.y - center.y)
    ...
    private func hashOf(_ data: [EmotionCategory: Int]) -> Int {
        var hasher = Hasher()
        for (key, value) in data.sorted(by: { $0.key.localizedName < $1.key.localizedName }) {
